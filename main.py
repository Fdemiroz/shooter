...
class Soldier(pygame.sprite.Sprite):
	...
	def move(self, moving_left, moving_right):
		...
		if self.jump == True and self.in_air == False:
			self.vel_y = -11 #how high the player jumps
			self.jump = False
			self.in_air = True
	...
	def update(self):
		...
		for enemy in enemy_group:
			if pygame.sprite.spritecollide(enemy, bullet_group, False):
				if enemy.alive:
					enemy.health -= 20 #damage per bullet
					self.kill()
...
class Explosion(pygame.sprite.Sprite):
	...
	def update(self):
		...
		if self.counter >= EXPLOSION_SPEED:
			self.counter = 0
			self.frame_index += 1
			#if animation is completed then delete the explosion
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.KEYUP:
			if event.key == pygame.K_a and pygame.K_LEFT:
				moving_left = False
			if event.key == pygame.K_d and pygame.K_RIGHT:
				moving_right = False
			if event.key == pygame.K_SPACE:
				shoot = False
			if event.key == pygame.K_q:
